...
abstract class QInstallationValidator {
	...
	//*/
		...
		// Email setup, as required for sEmailMessage
        if (strlen(__SMTP_SERVER__) == 0) {
            $obj = new QInstallationValidationResult();
            $obj->strMessage = "You need to have a SMTP server configured.";
            $obj->strCommandToFix = "Set the value for __SMTP_SERVER__ in configuration.inc.php";
            $result[] = $obj;
        }
        if (strlen(__SMTP_USER__) == 0) {
            $obj = new QInstallationValidationResult();
            $obj->strMessage = "You need to have a SMTP username configured.";
            $obj->strCommandToFix = "Set the value for __SMTP_USER____ in configuration.inc.php";
            $result[] = $obj;
        }
        if (strlen(__SMTP_PASSWORD__) == 0) {
            $obj = new QInstallationValidationResult();
            $obj->strMessage = "You need to have a SMTP password configured.";
            $obj->strCommandToFix = "Set the value for __SMTP_PASSWORD____ in configuration.inc.php";
            $result[] = $obj;
        }
		...
		for ($i = 1; $i < 1 + sizeof(QApplication::$Database); $i++) {
			...
			try {
				$db->Connect();
				// Ensure that the database is case sensitive
				if (!$db->checkTableNamesCase()) {
                    $obj = new QInstallationValidationResult();
                    $obj->strMessage = "Check your mysql configuration .ini file for \"lower_case_table_names\".";
                    $obj->strCommandToFix = "On WINDOWS this value should be 2. On other platforms it should be 0";
                    $result[] = $obj;
                }
			} catch (Exception $e) {
				$obj = new QInstallationValidationResult();
