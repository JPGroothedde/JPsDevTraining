<?php
	...
	abstract class QControlBase extends QBaseClass {
		...
		protected $blnIsBlockElement = false;
                ...
                // Added to accomodate Bootstrap styling
                protected $strControlLabelClass;
                protected $strControlWrapperClass;
                protected $blnRenderWithoutSpan = false;
                protected $blnIsCustomBlockElement = false;
...
//				$this->objParentControl->blnModified = true;
			...
			try {
                ...
                // Added to accomodate Bootstrap styling
                public function SetControlLabelCss($strCss) {
                    $this->strControlLabelClass = $strCss;
                }
                public function SetControlWrapperCss($strCss) {
                    $this->strControlWrapperClass = $strCss;
                }
                public function SetRenderWithoutSpan($blnRender = true) {
                    $this->blnRenderWithoutSpan = $blnRender;
                }
                public function SetRenderCustomBlock($blnRenderCustomBlock = true) {
                    $this->blnIsCustomBlockElement = $blnRenderCustomBlock;
                    $this->blnIsBlockElement = $blnRenderCustomBlock;
                }
                public function SetRenderBlock($blnRenderBlock = true) {
                    $this->blnIsBlockElement = $blnRenderBlock;                    
                }
		/**
		...
		protected function GetWrapperStyleAttributes($blnIsBlockElement=false) {
			$strStyle = '';
                        if (!$this->blnIsCustomBlockElement) {
...
//						}
				...
				default:
					...
					if ($this->blnUseWrapper) {
                                            if (!$this->blnRenderWithoutSpan){
						if ($blnIsBlockElement)
							$strOutput = sprintf('<div id="%s_ctl" %s%s>%s</div>%s', $this->strControlId, $strWrapperStyle, $strWrapperAttributes, $strOutput, $this->GetNonWrappedHtml());
						else
							$strOutput = sprintf('<span id="%s_ctl" %s%s>%s</span>%s', $this->strControlId, $strWrapperStyle, $strWrapperAttributes, $strOutput, $this->GetNonWrappedHtml());
                                            } else {
                                                $strOutput = sprintf('%s', $strOutput);
                                            }
                                        } else {
						$strOutput = $strOutput . $this->GetNonWrappedHtml();
					}
		...
		public function RenderWithName($blnDisplayOutput = true) {
			...
			$strToReturn = '<div class="form-group" ' . $strDataRel . '>';
			...
			$strToReturn .= sprintf('<label class="%s" for="%s">%s</label>%s', $this->strControlLabelClass, $this->strControlId, $this->strName, $strInstructions);
