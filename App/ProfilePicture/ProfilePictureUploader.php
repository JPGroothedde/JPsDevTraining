 ...
 * Time: 10:44
 */
class ProfilePictureUploader extends sFileUploader {
    ...
    public function HandleDocumentUpload($strFormId, $strControlId, $strParameter/*, $RefObj*/) {
        ...
        if (isset($_SESSION['FileUploaded'])) {
            ...
            foreach ($fileUploadArray as $uploadedFile) {
                ...
                $ProfilePictureObj = new ProfilePicture();
                $ProfilePictureObj->AccountObject = Account::Load(AppSpecificFunctions::getCurrentAccountAttribute());
                $ProfilePictureObj->ProfilePicturePath = __FILE_UPLOADED_PATH__.$uploadedFile;
                try {
                    $theDocument->Save();
                    $ProfilePictureObj->FileDocumentObject = FileDocument::Load('Id');
                    $ProfilePictureObj->Save();
