 ...
 */
	...
	function QcodoHandleCodeGenParseError($__exc_errno, $__exc_errstr, $__exc_errfile, $__exc_errline) {
		...
		sDev_CodeGenerator::$RootErrors .= sprintf("%s\r\n", $strErrorString);
	}
	...
	function GO_BACK($intNumChars) {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            $content_so_far = str_replace("\r", '', $content_so_far);
        }
	 ...
	 * @package Codegen
	 */
	abstract class sDev_CodeGeneratorBase extends QBaseClass {
...
//		const SubTemplatesPath = '/codegen/subtemplates/';
		...
		const TemplatesPathCustom = '/templates/';
//		const SubTemplatesPathCustom = '/codegen/subtemplates/';
		...
		public static function GetSettingsXml() {
			...
			$strToReturn .= sprintf('	<name application="%s"/>%s', sDev_CodeGenerator::$ApplicationName, $strCrLf);
			$strToReturn .= sprintf('	<templateEscape begin="%s" end="%s"/>%s', sDev_CodeGenerator::$TemplateEscapeBegin, sDev_CodeGenerator::$TemplateEscapeEnd, $strCrLf);
		...
		public static function Run($strSettingsXmlFilePath) {
			sDev_CodeGenerator::$CodeGenArray = array();
			sDev_CodeGenerator::$SettingsFilePath = $strSettingsXmlFilePath;

			if (!file_exists($strSettingsXmlFilePath)) {
				sDev_CodeGenerator::$RootErrors = 'FATAL ERROR: CodeGen Settings XML File (' . $strSettingsXmlFilePath . ') was not found.';
				return;
			}
			...
			try {
				...
				sDev_CodeGenerator::$SettingsXml = new SimpleXMLElement(file_get_contents($strSettingsXmlFilePath));
				QApplication::RestoreErrorHandler();
			} catch (Exception $objExc) {
				sDev_CodeGenerator::$RootErrors .= 'FATAL ERROR: Unable to parse CodeGenSettings XML File: ' . $strSettingsXmlFilePath;
				sDev_CodeGenerator::$RootErrors .= "\r\n";
				sDev_CodeGenerator::$RootErrors .= $objExc->getMessage();
				return;
			}

			// Set the Template Escaping
			sDev_CodeGenerator::$TemplateEscapeBegin = sDev_CodeGenerator::LookupSetting(sDev_CodeGenerator::$SettingsXml, 'templateEscape', 'begin');
			sDev_CodeGenerator::$TemplateEscapeEnd = sDev_CodeGenerator::LookupSetting(sDev_CodeGenerator::$SettingsXml, 'templateEscape', 'end');
			sDev_CodeGenerator::$TemplateEscapeBeginLength = strlen(sDev_CodeGenerator::$TemplateEscapeBegin);
			sDev_CodeGenerator::$TemplateEscapeEndLength = strlen(sDev_CodeGenerator::$TemplateEscapeEnd);

			if ((!sDev_CodeGenerator::$TemplateEscapeBeginLength) || (!sDev_CodeGenerator::$TemplateEscapeEndLength)) {
				sDev_CodeGenerator::$RootErrors .= "CodeGen Settings XML Fatal Error: templateEscape begin and/or end was not defined\r\n";
				return;
			}

			// Application Name
			sDev_CodeGenerator::$ApplicationName = sDev_CodeGenerator::LookupSetting(sDev_CodeGenerator::$SettingsXml, 'name', 'application');

			// Iterate Through DataSources
			if (sDev_CodeGenerator::$SettingsXml->dataSources->asXML())
				foreach (sDev_CodeGenerator::$SettingsXml->dataSources->children() as $objChildNode) {
					switch (dom_import_simplexml($objChildNode)->nodeName) {
						case 'database':
							sDev_CodeGenerator::$CodeGenArray[] = new QDatabaseCodeGen($objChildNode);
							break;
						case 'restService':
							sDev_CodeGenerator::$CodeGenArray[] = new QRestServiceCodeGen($objChildNode);
							break;
						default:
							sDev_CodeGenerator::$RootErrors .= sprintf("Invalid Data Source Type in CodeGen Settings XML File (%s): %s\r\n",
...
//			array_push($strToReturn, QRestServiceCodeGen::GenerateAggregateHelper($objRestServiceCodeGen));
		 ...
		 * all the _*.tpl templates from any subfolders of the template prefix in sDev_CodeGenerator::TemplatesPath and sDev_CodeGenerator::TemplatesPathCustom,
		...
		public function GenerateFiles($strTemplatePrefix, $mixArgumentArray) {
			...
			$strTemplatePath = sprintf('%s%s%s', __SDEV_ORM__.'/sDev_CodeGenerator/_core' , sDev_CodeGenerator::TemplatesPath, $strTemplatePrefix);
			if (!is_dir($strTemplatePath))
				throw new Exception(sprintf("sDev_CodeGenerator::TemplatesPath does not appear to be a valid directory:\r\n%s", $strTemplatePath));

			$strTemplatePathCustom = sprintf('%s%s', __SDEV_ORM__.'/sDev_CodeGenerator/_core', sDev_CodeGenerator::TemplatesPathCustom);
			if (!is_dir($strTemplatePathCustom))
				throw new Exception(sprintf("sDev_CodeGenerator::TemplatesPathCustom does not appear to be a valid directory:\r\n%s", $strTemplatePathCustom));
			...
			while ($strModuleName = readdir($objDirectory)) {
				if (!in_array(strtolower($strModuleName), sDev_CodeGenerator::$DirectoriesToExcludeArray) &&
		...
		public function GenerateFile($strModuleName, $strFilename, $blnOverrideFlag, $mixArgumentArray, $blnSave = true) {
			...
			if ($blnOverrideFlag)
				$strTemplateFilePath = __SDEV_ORM__.'/sDev_CodeGenerator/_core' . sDev_CodeGenerator::TemplatesPathCustom . $strModuleName . '/' . $strFilename;
			else
				$strTemplateFilePath = __SDEV_ORM__.'/sDev_CodeGenerator/_core' . sDev_CodeGenerator::TemplatesPath . $strModuleName . '/' . $strFilename;

			// Setup Debug/Exception Message
			if (sDev_CodeGenerator::DebugMode) _p("Evaluating $strTemplateFilePath<br/>", false);
			...
			if (substr($strFilename, strlen($strFilename) - 8) == '.tpl.php')  {
				...
				$strSearchPath = __SDEV_ORM__.'/sDev_CodeGenerator/_core' . sDev_CodeGenerator::TemplatesPathCustom . $strModuleName . PATH_SEPARATOR .
                    __SDEV_ORM__.'/sDev_CodeGenerator/_core' . sDev_CodeGenerator::TemplatesPath . $strModuleName . PATH_SEPARATOR .
						get_include_path();
		...
		protected function EvaluateSubTemplate($strSubTemplateFilename, $strModuleName, $mixArgumentArray) {
			if (sDev_CodeGenerator::DebugMode) _p("Evaluating $strSubTemplateFilename<br/>", false);

			// Try the Custom SubTemplate Path (PHP template version)
			$strFilename = sprintf('%s%s%s/%s.php', __SDEV_ORM__.'/sDev_CodeGenerator/_core', sDev_CodeGenerator::TemplatesPathCustom, $strModuleName, $strSubTemplateFilename);
			if (file_exists($strFilename))
				return $this->EvaluatePHP($strFilename, $strModuleName, $mixArgumentArray);

			// Try the Custom SubTemplate Path
			$strFilename = sprintf('%s%s%s/%s', __SDEV_ORM__.'/sDev_CodeGenerator/_core', sDev_CodeGenerator::TemplatesPathCustom, $strModuleName, $strSubTemplateFilename);
			if (file_exists($strFilename))
			...
			$strFilename = sprintf('%s%s%s/%s', __SDEV_ORM__.'/sDev_CodeGenerator/_core', sDev_CodeGenerator::TemplatesPath, $strModuleName, $strSubTemplateFilename);
			if (file_exists($strFilename))
				return $this->EvaluateTemplate(file_get_contents($strFilename), $strModuleName, $mixArgumentArray);

			// Try the Standard SubTemplate Path (PHP template version)
			$strFilename = sprintf('%s%s%s/%s.php', __SDEV_ORM__.'/sDev_CodeGenerator/_core', sDev_CodeGenerator::TemplatesPath, $strModuleName, $strSubTemplateFilename);
			if (file_exists($strFilename))
				return $this->EvaluatePHP($strFilename, $strModuleName, $mixArgumentArray);
		...
		protected function EvaluateTemplate($strTemplate, $strModuleName, $mixArgumentArray) {
			...
			$intPosition = strpos($strTemplate, sDev_CodeGenerator::$TemplateEscapeBegin);
			...
			while ($intPosition !== false) {
				$intPositionEnd = strpos($strTemplate, sDev_CodeGenerator::$TemplateEscapeEnd, $intPosition);

				// Get and cleanup the Eval Statement
				$strStatement = substr($strTemplate, $intPosition + sDev_CodeGenerator::$TemplateEscapeBeginLength,
										$intPositionEnd - $intPosition - sDev_CodeGenerator::$TemplateEscapeEndLength);
				...
				// Check if we're starting an open-ended statemen
					...
					$strSubTemplate = substr($strTemplate, $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength);

					// Parse through the rest of the template, and pull the correct SubTemplate,
					// Keeping in account nested open-ended statements
					$intLevel = 1;

					$intSubPosition = strpos($strSubTemplate, sDev_CodeGenerator::$TemplateEscapeBegin);
					while (($intLevel > 0) && ($intSubPosition !== false)) {
						$intSubPositionEnd = strpos($strSubTemplate, sDev_CodeGenerator::$TemplateEscapeEnd, $intSubPosition);
						$strFragment = substr($strSubTemplate, $intSubPosition + sDev_CodeGenerator::$TemplateEscapeEndLength,
							$intSubPositionEnd - $intSubPosition - sDev_CodeGenerator::$TemplateEscapeEndLength);
						...
						if ($intLevel)
							$intSubPosition = strpos($strSubTemplate, sDev_CodeGenerator::$TemplateEscapeBegin, $intSubPositionEnd);
					}
					if ($intLevel != 0)
					...
					$intPositionEnd = $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength + $intSubPositionEnd;

					// If nothing but whitespace between $intPositionEnd and the next CR, then remove the CR
					$intCrPosition = strpos($strTemplate, "\n", $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength);
					if ($intCrPosition !== false) {
						$strFragment = substr($strTemplate, $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength, $intCrPosition - ($intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength));
						if (trim($strFragment == '')) {
							// Nothing exists after the escapeend and the next CR
							// Go ahead and chop it off
							$intPositionEnd = $intCrPosition - sDev_CodeGenerator::$TemplateEscapeEndLength + 1;
						}
					} else {
						$strFragment = substr($strTemplate, $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength);
						if (trim($strFragment == '')) {
					...
					if (is_null($strStatement))
						...
						if (sDev_CodeGenerator::DebugMode) _p("Evalling: $strStatement<br/>", false);
						// Perform the Eval
				...
				// Do the Replace
				$strTemplate = substr($strTemplate, 0, $intPosition) . $strStatement . substr($strTemplate, $intPositionEnd + sDev_CodeGenerator::$TemplateEscapeEndLength);
		...
		public static function MetaControlLabelNameFromColumn (QColumn $objColumn) {
			...
			foreach (sDev_CodeGenerator::$CodeGenArray as $DatabaseCodeGen) {
