...
class Post_OverviewForm extends QForm {
    ...
    protected function btnDeletePost_Clicked($strFormId, $strControlId, $strParameter) {
        ...
        $EmailAddress = $theObject->AccountObject->EmailAddress;
        $EmailMessage = 'One of your post\'s was deleted by a admin, details are as follows:<br/>';
        $EmailMessage.= 'Date and time of post:'.$theObject->PostTimeStamp->format(DATE_TIME_FORMAT_HTML.' H:i:s').'<br/>';
        $EmailMessage.= 'Post:'.$theObject->PostText.'<br/>';
        $SendEmailMessage = new sEmailMessage(array($EmailAddress),"Post Deletion",$EmailMessage);
        if ($SendEmailMessage->sendMail()) {
            AppSpecificFunctions::AddCustomLog($EmailMessage);
        } else {
            AppSpecificFunctions::AddCustomLog('Email not sent....');
        }
    ...
    protected function InitPostDataGrid() {
        ...
        $headerItems = array('Time Created', 'Post Text', 'Account');
        $headerSortNodes = array(QQN::Post()->PostTimeStamp,QQN::Post()->PostText,  QQN::Post()->AccountObject->Id);
        $columnItems = array('PostTimeStamp','PostText',  'Account');
        $QueryConditions = '';
        ...
        $this->PostGrid->SetSortDirectionDown(true);
    }
