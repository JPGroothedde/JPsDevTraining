...
class PayGate_PayWeb3{
	...
	/**
	 * @var string the url of the PayGate PaySubs process page
	 */
	static public $process_url_subs  = 'https://www.paygate.co.za/PaySubs/process.trans';
	/**
	...
	function __construct($IsSub = false){
		$this->IsSub = $IsSub;
	}
	...
	function getTransactionStatusDescription($statusNumber){
		if (array_key_exists($statusNumber,$this->transactionStatusArray))
			return $this->transactionStatusArray[$statusNumber];
		return 'Unknown payment status: '.$statusNumber;
	}
	...
	public function generateChecksum($postData){
		...
		AppSpecificFunctions::AddCustomLog('Checksum str: '.$checksum);
		if($this->isDebug()){
	...
	public function doInitiate(){
		...
		if ($this->IsSub) {
			return $this->initiateRequest['CHECKSUM'];
		}
