<?php
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QColumn.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QIndex.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QManyToManyReference.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QReference.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QReverseReference.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QTable.class.php');
	require(__SDEV_ORM__ . '/sDev_CodeGenerator/_core/QTypeTable.class.php');


	class QDatabaseCodeGen extends sDev_CodeGenerator {
		// Objects
		...
		public function __construct($objSettingsXml) {
			...
			$this->intDatabaseIndex = sDev_CodeGenerator::LookupSetting($objSettingsXml, null, 'index', QType::Integer);

			// Append Suffix/Prefixes
			$this->strClassPrefix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'className', 'prefix');
			$this->strClassSuffix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'className', 'suffix');
			$this->strAssociatedObjectPrefix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'associatedObjectName', 'prefix');
			$this->strAssociatedObjectSuffix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'associatedObjectName', 'suffix');

			// Table Type Identifiers
			$strTypeTableSuffixList = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'typeTableIdentifier', 'suffix');
			...
			$this->strAssociationTableSuffix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'associationTableIdentifier', 'suffix');
			$this->intAssociationTableSuffixLength = strlen($this->strAssociationTableSuffix);

			// Stripping TablePrefixes
			$this->strStripTablePrefix = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'stripFromTableName', 'prefix');
			$this->intStripTablePrefixLength = strlen($this->strStripTablePrefix);

			// Exclude/Include Tables
			$this->strExcludePattern = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'excludeTables', 'pattern');
			$strExcludeList = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'excludeTables', 'list');
			$this->strExcludeListArray = explode(',',$strExcludeList);
			array_walk($this->strExcludeListArray, 'array_trim');

			// Include Patterns
			$this->strIncludePattern = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'includeTables', 'pattern');
			$strIncludeList = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'includeTables', 'list');
			$this->strIncludeListArray = explode(',',$strIncludeList);
			array_walk($this->strIncludeListArray, 'array_trim');

			// ManualQuery Support
			$this->blnManualQuerySupport = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'manualQuery', 'support', QType::Boolean);

			// Relationship Scripts
			$this->strRelationships = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'relationships');
			$this->strRelationshipsScriptPath = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'relationshipsScript', 'filepath');
			$this->strRelationshipsScriptFormat = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'relationshipsScript', 'format');

			// Column Comment for MetaControlLabel setting.
			$this->strCommentMetaControlLabelDelimiter = sDev_CodeGenerator::LookupSetting($objSettingsXml, 'columnCommentForMetaControl', 'delimiter');
...
//				$this->objTableArray[strtolower($strTableWithReference)]->ManyToManyReferenceArray = $objArray;
		...
		protected function AnalyzeTypeTable(QTypeTable $objTypeTable) {
			...
			$strReservedWords = explode(',', sDev_CodeGenerator::PhpReservedWords);
			...
			while ($objRow = $objResult->FetchRow()) {
				...
				if (sizeof($objRow) > 2) { // there are extra columns to process
					...
					for ($i = 2; $i < sizeof($objRow); $i++) {
						$strFieldName = sDev_CodeGenerator::TypeNameFromColumnName($objFieldArray[$i]->Name);
