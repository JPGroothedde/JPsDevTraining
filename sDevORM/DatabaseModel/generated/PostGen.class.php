<?php
	 ...
	 * @property-read Comment $_Comment the value for the private _objComment (Read-Only) if set due to an expansion on the Comment.Post reverse relationship
	 * @property-read Comment[] $_CommentArray the value for the private _objCommentArray (Read-Only) if set due to an ExpandAsArray on the Comment.Post reverse relationship
	...
	class PostGen extends QBaseClass implements IteratorAggregate {
		...
		const PostTextMaxLength = 200;
		const PostTextDefault = null;


		/**
		 * Protected member variable that maps to the database column Post.DateCreated
		...
		public function Initialize()
		{
			...
			$this->dttDateCreated = (Post::DateCreatedDefault === null)?null:new QDateTime(Post::DateCreatedDefault);
		...
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $objExpandAsArrayNode = null, $objPreviousItemArray = null, $strColumnAliasArray = array()) {
			...
			$strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
			$objExpansionNode = (empty($objExpansionAliasArray['comment']) ? null : $objExpansionAliasArray['comment']);
			$blnExpanded = ($objExpansionNode && $objExpansionNode->ExpandAsArray);
			if ($blnExpanded && null === $objToReturn->_objCommentArray)
				$objToReturn->_objCommentArray = array();
			if (!is_null($objDbRow->GetColumn($strAliasName))) {
				if ($blnExpanded) {
					$objToReturn->_objCommentArray[] = Comment::InstantiateDbRow($objDbRow, $strAliasPrefix . 'comment__', $objExpansionNode, null, $strColumnAliasArray);
				} elseif (is_null($objToReturn->_objComment)) {
					$objToReturn->_objComment = Comment::InstantiateDbRow($objDbRow, $strAliasPrefix . 'comment__', $objExpansionNode, null, $strColumnAliasArray);
				}
			}
...
*/
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case '_Comment':
					 ...
					 * if set due to an expansion on the Comment.Post reverse relationship
					 * @return Comment
					 */
				...
				case '_CommentArray':
					 ...
					 * if set due to an ExpandAsArray on the Comment.Post reverse relationship
					 * @return Comment[]
					 */
		...
		*/
		public function GetCommentArray($objOptionalClauses = null) {
			...
			try {
				return Comment::LoadArrayByPost($this->intId, $objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		*/
		public function CountComments() {
			...
			return Comment::CountByPost($this->intId);
		}
	...
	class QQNodePost extends QQNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'Comment':
					return new QQReverseReferenceNodeComment($this, 'comment', 'reverse_reference', 'Post', 'Comment');
				case 'PostLike':
