<?php
	...
	abstract class QDataGridBase extends QPaginatedControl {
		...
		protected $btnFilterResetArray = array();
                ...
                protected $blnUseCollapseFilter;
                protected $strSimpleHTMLAttr;

                protected $blnUseCustomHeader;
                protected $strCustomHeaderHTML;

                public function setCollapseFilter($blnCollapse = true) {
                    $this->blnUseCollapseFilter = $blnCollapse;
                }
                public function setUseCustomHeader($blnUseCustomHeader = true) {
                    $this->blnUseCustomHeader = $blnUseCustomHeader;
                }
                public function setCustomHeaderHtml($html = '') {
                    $this->strCustomHeaderHTML = $html;
                }
			...
			foreach ($this->objColumnArray as $objColumn) {
				try {
					...
					throw $objExc;
                                ...
                                if ($intCountCols = $this->SortColumnIndex)
                                        $strColumnsHtml .= sprintf("    <td class=\"info\" %s>%s</td>\r\n", $objColumn->GetAttributes(), $strHtml);
                                else
		...
		protected function GetControlHtml() {
			...
			if ($strStyle)
				$strStyle = sprintf('style="%s" ', $strStyle);
                        if ($this->blnUseCustomHeader) {
                            $strToReturn = "";
                        } else
                        {
                           if ($this->blnUseCollapseFilter)
                            {
                               $strToReturn = sprintf('
                                    <button type="button" class="btn btn-link pull-right" data-toggle="collapse" data-target="#collapse%s">
                                    Toggle Search
                                    </button>
                                <table id="%s" %s%s>'."\r\n", $this->strControlId, $this->strControlId, $this->GetAttributes(), $strStyle);
                            }else
                            {
		...
		*/
		protected function GetFilterRowHtml() {
			$objFilterStyle = $this->objRowStyle->ApplyOverride($this->objFilterRowStyle);
                        $strCollapse = '';
                        if ($this->blnUseCollapseFilter)
                            $strCollapse = 'id = "collapse'.$this->strControlId.'" class="collapse"';
			$strToReturn = sprintf(' <tr %s %s>'."\r\n", $objFilterStyle->GetAttributes(),
                                    $strCollapse);
		...
		**/
		protected function filterTextBox_Create($strControlId, $strControlName, $columns, $strValue) {
			...
			$ctlFilterTextBox->Columns = $columns;
                        $ctlFilterTextBox->CssClass = 'filterTextBox';
		...
		**/
		...
		protected function btnFilterReset_Create() {
			...
			$btnFilterReset->Text = QApplication::Translate('Reset');;
                        $btnFilterReset->AddCssClass('btn-mrgleft');
		...
		protected function btnFilter_Create()
		{
			...
			$btnFilter->Text = QApplication::Translate('Filter');
                        $btnFilter->AddCssClass('btn-mrgleft');
