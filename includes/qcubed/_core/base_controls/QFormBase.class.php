<?php
	...
	abstract class QFormBase extends QBaseClass {
		...
		// Default javascript that is called when the enter key is pressed
		protected $defaultEnterPressedJs = 'event.preventDefault();';
...
//					$strToReturn .= $objControl->RenderAjax(false) . "\r\n";
		...
		/**
		 * @param string $js The javascript to execute whenever the enter key is pressed.
		 *
		 * @return null
		 */
		public function SetDefaultEnterPressedJs($js) {
			$this->defaultEnterPressedJs = $js;
		}
		...
		public function RenderEnd($blnDisplayOutput = true) {
			...
			$strEnterKeyCatch = sprintf('<script type="text/javascript">$j(document).keypress(function(e) {
											if(e.which == 13 && e.target.nodeName != "TEXTAREA") {
												%s
											}
										});</script>',$this->defaultEnterPressedJs);
			...
			$strToReturn .= $strEndScript.$strEnterKeyCatch;
