<?php
	...
	class QRadioButtonList extends QListControl {
		...
		protected function GetItemHtml($objItem, $intIndex, $strActions, $strTabIndex) {
			...
			if (!$this->blnEnabled) {
				$strToReturn .= '<span disabled="disabled">';
                        ...
                        $isActiveCss = ($objItem->Selected) ? 'active' : '';
			$strLabel = sprintf('<label class="btn btn-primary '.$isActiveCss.'" for="%s">%s',
				$strIndexedId,
		...
		protected function GetControlHtml() {
			...
			if ($this->strCssClass)
				$strCssClass = sprintf('class="btn-group %s" data-toggle="buttons"', $this->strCssClass);
			else
				$strCssClass = 'class="btn-group" data-toggle="buttons"';
			...
			if (strlen($strStyle) > 0)
				$strStyle = sprintf('style="display:block;%s" ', $strStyle);
                        else
                            $strStyle = sprintf('style="display:block;" ');
			...
			if ($this->intButtonMode == self::ButtonModeSet) {
				...
				return $strToReturn;
                        ...
                        //Forcing Button mode since the old school thing is stupid
                        $strToReturn = sprintf('<div id="%s" %s%s%s%s%s>',
                                $this->strControlId,
                                $strAccessKey,
                                $strToolTip,
                        ...
                        $strToReturn .= '</div>';
                        if (strlen($this->strHelpText) > 0)
                            $strToReturn .= '<span class="help-block pull-left">'.$this->strHelpText.'</span>';
                        return $strToReturn;
                        /*
			// Generate Table HTML
