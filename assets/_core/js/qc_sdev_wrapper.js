// This file is a wrapper for the qcubed javascript base
// Helpers
	...
	var qcubed = {
		...
		postAjax: function(strForm, strControl, strEvent, mixParameter, strWaitIconControlId) {
			if (mustCheckConnection) {
				checkConnection(0,testConnectionCallback,strForm, strControl, strEvent, mixParameter, strWaitIconControlId);
			} else {
				//Ignore checking for online status and just do the post. If you want to use this implementation, simply comment out the two lines above
				PostAjaxWrapper(true, strForm, strControl, strEvent, mixParameter, strWaitIconControlId);
			}
...
/////////////////////////////
	...
	function testConnectionCallback(online, strForm, strControl, strEvent, mixParameter, strWaitIconControlId) {
		PostAjaxWrapper(online, strForm, strControl, strEvent, mixParameter, strWaitIconControlId);
	};

	function PostAjaxWrapper(online,strForm, strControl, strEvent, mixParameter, strWaitIconControlId) {
	//console.log('Is online: '+online);
	if (!online) {
		alert('You are currently not connected to the server. Please check your connection and try again.');
		removeAjaxOverlay();
	...
	if (online) {
		...
		// This is part of QCubed and is not used by sDev
		...
		$j.ajaxQueue({
			...
			error: function (XMLHttpRequest, textStatus, errorThrown) {
				...
				window.open(getErrorPage(), "_self");
				return false;
			...
			success: function (xml) {
				...
				// This is not working on Firefox... Figure out why...
