<?php
	...
	class QAjaxAction extends QAction {
        ...
        /**
		 * @var string The value of the Control's Text while the action is performed
		 */
		protected $strControlText;
        /**
         * @var string Indicates whether to show a progressbar or not
         */
        protected $blnShowProgressBar;
		 ...
		 *              client-side when the action occurs!
		 */
		public function __construct($strMethodName = null, $objWaitIconControl = 'default', $mixCausesValidationOverride = null,$strJsReturnParam = "",$strControlText = null,$blnShowProgressBar = true) {
			$this->strId = NULL;
			$this->strMethodName = $strMethodName;
			$this->objWaitIconControl = $objWaitIconControl;
			$this->mixCausesValidationOverride = $mixCausesValidationOverride;
			$this->strJsReturnParam = $strJsReturnParam;
            $this->strControlText = $strControlText;
            $this->blnShowProgressBar = $blnShowProgressBar;
		}
		...
		public function RenderScript(QControl $objControl) {
            ...
            if ($this->strControlText && $this->blnShowProgressBar) {
                return sprintf("showAjaxOverlay();$(\"#%s\").attr(\"disabled\", true);$(\"#%s\").html('%s');qc.pA('%s', '%s', '%s#%s', %s, '%s');",
                        $objControl->ControlId, $objControl->ControlId, $this->strControlText, $objControl->Form->FormId, $objControl->ControlId, get_class($this->objEvent), $this->strId, $this->getActionParameter($objControl), $strWaitIconControlId);
            } elseif ($this->strControlText) {
                return sprintf("$(\"#%s\").attr(\"disabled\", true);$(\"#%s\").html('%s');qc.pA('%s', '%s', '%s#%s', %s, '%s');",
                    $objControl->ControlId, $objControl->ControlId, $this->strControlText, $objControl->Form->FormId, $objControl->ControlId, get_class($this->objEvent), $this->strId, $this->getActionParameter($objControl), $strWaitIconControlId);
            } elseif ($this->blnShowProgressBar) {
                return sprintf("showAjaxOverlay();qc.pA('%s', '%s', '%s#%s', %s, '%s');",
